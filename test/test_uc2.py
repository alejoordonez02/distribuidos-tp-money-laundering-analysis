    ...
    for n in range(NCLIENTS):
        ...
        with open(
            ...
            while line := expected_responses.readline():
                ...
                expected.add(Result(bank_id, bank_name, float(amount)))
        ...
        with open(CLIENT_RESPONSES_PATH + f"responses_{n}.csv", "r") as responses:
            ...
            while line := responses.readline():
                if "--- UC" in line:
                    break
                m = re.match(
                    r"bank_id: (\S+)\s+account: (\S+)\s+bank_name: (.*?)\s+amount: (\S+)$",
                    line.strip(),
                )
                bank_id, account, bank_name, amount = (
                    m.group(1),
                    m.group(2),
                    m.group(3),
                    m.group(4),
                )
                got.add(Result(bank_id, bank_name, float(amount)))
