...
class UC2BankIdMergeFn(MergeFn):
    ...
    def __init__(self):
        ...
        self._max_amounts: dict[UUID, dict[str, tuple[str, float]]] = {}
        # client_id → {bank_id → bank_name}
        self._bank_names: dict[UUID, dict[str, str]] = {}

    def left(self, msg: MaxByBank):  # type: ignore[reportIncompatibleMethodOverride]
        if msg.client_id not in self._max_amounts:
            self._max_amounts[msg.client_id] = {}
        state = self._max_amounts[msg.client_id]
    ...
    def right(self, msg: BankNames):  # type: ignore[reportIncompatibleMethodOverride]
        if msg.client_id not in self._bank_names:
            self._bank_names[msg.client_id] = {}
        self._bank_names[msg.client_id].update(msg.data)

    def get_result(self, client_id: UUID) -> MergedBankData:
        max_amounts = self._max_amounts.get(client_id, {})
        bank_names = self._bank_names.get(client_id, {})
        entries = [
