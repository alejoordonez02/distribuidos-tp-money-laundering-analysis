...
class StatefulRingEOFHandler(StatefulEOFHandler, RingEOFHandler):
    def __init__(self, id2: int, mom_ring: MOMRing, txs: list[MOMQueue]):
        self.id = id2
    ...
    def _handle_ring_eof(self, bytes2: bytes, ack: Callable, _: Callable):
        ...
        with self.mtx:
            ...
            if eof.processed_count == eof.expected_count and eof.origin == self.id:
