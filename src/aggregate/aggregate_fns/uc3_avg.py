...
class UC3AvgAggregateFn(AggregateFn):
    ...
    def aggregate(self, msg: SumByPaymentFormat):  # type: ignore[reportIncompatibleMethodOverride]
        ...
        for (format, (total_sum, transactions_amount)) in msg.data.items():
    ...
    def get_result(self, client_id: UUID) -> AvgByFormat:
        response = {}
        for (format, (total_sum, transactions_amount)) in self._state.get(client_id, AvgByFormat(client_id, {})).data.items():
